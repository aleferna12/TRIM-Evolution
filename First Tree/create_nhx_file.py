...
NEWICK_PATH = "upto_600aa_clustalo_iter2.fasta.tree"
CSV_SEQ_INFO_PATH = "../cluster_seqs.csv"
DEST_PATH = "upto_600aa_clustalo_iter2.tree"
...
def main():
	...
	for node in t.iter_descendants():
		if node.is_leaf():
			...
			sh_alrt, support = node.name.split('/')
			node.add_features(sh_alrt=float(sh_alrt), support=float(support))
			node.name = ""

	t.write(outfile=DEST_PATH, format=0, features=["species", "gene_id", "trim", "sh_alrt"])
